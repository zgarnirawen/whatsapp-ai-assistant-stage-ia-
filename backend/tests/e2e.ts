...
type Scenario = {
  id: string;
  name: string;
  run: (client: E2EClient) => Promise<void>;
};
...
class E2EClient {
  ...
  async message(inputText: string, inputMode = "text") {
    const result = await this.request("/assistant/message", {
      method: "POST",
      body: JSON.stringify({ inputText, inputMode }),
    });
  ...
  async confirm(payload: Record<string, unknown>) {
    const result = await this.request("/assistant/confirm-action", {
      method: "POST",
...
function assertIntent(result: any, ...expected: string[]) {
  assert(expected.includes(result.body?.result?.intent), `expected intent ${expected.join(" or ")}, got ${result.body?.result?.intent}`);
}
...
const scenarios: Scenario[] = [
  ...
  { id: "E2E-14", name: "Modify existing task", run: async c => { const r = await c.message("Renomme la tâche E2E unique à tester en E2E tâche modifiée"); assertIntent(r, "modify_task"); assert(r.body.proposedAction?.targetId, "modify task target was not resolved"); } },
  ...
  { id: "E2E-16", name: "Create then modify event", run: async c => { const r = await c.message("Crée un événement E2E réunion à tester demain à 15h"); assertIntent(r, "create_event"); const confirmed = await c.confirm({ interactionId: r.body.interaction.id, intent: r.body.proposedAction.intent, details: r.body.proposedAction.details, targetId: null }); const id = confirmed.body.createdItem?.id; assert(id, "event was not created"); const m = await c.message("Change l'événement E2E réunion à tester en E2E réunion modifiée"); assertIntent(m, "modify_event"); assert(m.body.proposedAction?.targetId === id, "modify event target mismatch"); } },
  ...
  { id: "E2E-18", name: "Not-found modification target", run: async c => { const r = await c.message("Supprime la tâche qui n'existe vraiment pas E2E"); assertIntent(r, "delete_task"); assert(r.body.modifyDeleteInfo?.status === "not_found", "expected not_found target status"); } },
...
async function main() {
  ...
  const report = {
    ...
    passed,
    failed,
    errorRatePercent: Number(((failed / scenarios.length) * 100).toFixed(1)),
    latencyMs: { min: Math.min(...durations), average: Math.round(durations.reduce((a, b) => a + b, 0) / Math.max(durations.length, 1)), p95: percentile(95), max: Math.max(...durations) },
    results,
  };
